...
- **Rationale:** Qatar and GCC fleet managers expect Arabic as a first-class language. Testing confirmed BUG-002: no language switcher exists. This is rated Medium priority for test execution (tests still pass in English), but the underlying defect is Critical for go-live readiness.
